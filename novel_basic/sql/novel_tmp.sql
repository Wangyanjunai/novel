...
CREATE TABLE `novel_advertisement` (
  ...
  `image_url` varchar(1256) DEFAULT NULL COMMENT '广告图片链接绝对路径URL。',
  `link_url` varchar(1256) DEFAULT NULL COMMENT '广告跳转链接绝对路径URL。',
...
CREATE TABLE `novel_category` (
  ...
  `category_zh_name` varchar(64) DEFAULT NULL COMMENT '中文名称',
...
CREATE TABLE `novel_info` (
  ...
  `introduction` varchar(4096) DEFAULT NULL COMMENT '小说简介',
  ...
  `last_chapter_id` varchar(32) DEFAULT NULL COMMENT '最新章节id',
  `last_chapter_title` varchar(256) DEFAULT NULL COMMENT '最新章节标题（名称）',
  `total_chapters` int(6) unsigned DEFAULT '0' COMMENT '章节总数',
  `retention` smallint(3) unsigned DEFAULT '0' COMMENT '留存率，现在只是保存数字，显示的时候加上百分比',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
