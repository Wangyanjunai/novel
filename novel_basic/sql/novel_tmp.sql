...
CREATE TABLE `novel_chapter` (
  `chapter_id` varchar(32) NOT NULL COMMENT '章节id，主键。',
  ...
  `chapter_url` varchar(1024) NULL DEFAULT NULL COMMENT '章节数据路径URL。',
...
CREATE TABLE `novel_info` (
  ...
  `data_url` varchar(1024) NULL DEFAULT NULL COMMENT '小说信息数据的地址url。',
  ...
  `novel_status` tinyint(1) UNSIGNED NOT NULL DEFAULT 1 COMMENT '状态，0-已完结；1-连载中，默认：1-连载中。',
  `category_type` smallint(4) UNSIGNED NOT NULL COMMENT '类型编号。',
  `category_cn_text` varchar(64) NULL DEFAULT NULL COMMENT '中文名称。',
  `category_en_text` varchar(64) NULL DEFAULT NULL COMMENT '英文名称。',
  ...
  `newest_chapter_title` varchar(512) NULL DEFAULT NULL COMMENT '最新章节标题（名称）。',
  `total_chapters` int(6) UNSIGNED NULL DEFAULT 0 COMMENT '章节总数，默认：0。',
  `retention` tinyint(3) UNSIGNED NULL DEFAULT 0 COMMENT '留存率，现在只是保存数字，显示的时候加上百分比，默认：0。',
  `create_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '创建时间。',
  `update_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '更新时间。',
  PRIMARY KEY (`id`) USING BTREE,
